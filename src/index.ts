...
const pems = selfsigned.generate(
	[
		...
		{ name: 'subjectAltName', value: [{ type: 7, value: '0.0.0.0' }] },
	],
	{ days: 365, algorithm: 'ed25519', keySize: 2048 }
);
	...
	static load(
		...
		return new Promise((resolve) => {
			...
			leaderTpuCache.connection.getEpochInfo().then((epochInfo) => {
				...
				leaderTpuCache
					.fetchSlotLeaders(
						leaderTpuCache.first_slot,
						leaderTpuCache.slots_in_epoch
					)
					.then((leaders) => {
...
export class RecentLeaderSlots {
	...
	estimatedCurrentSlot(): number {
		...
		const expected_current_slot =
			median_recent_slot + (max_index - median_index);
		const max_reasonable_current_slot =
			expected_current_slot + MAX_SLOT_SKIP_DISTANCE;
...
export class TpuClient {
	...
	private constructor(
		...
		this.connection = connection;
		this.exit = false;
		this.fanoutSlots = Math.max(
			Math.min(config.fanoutSlots, MAX_FANOUT_SLOTS),
			1
		);
	...
	static load(
		...
		return new Promise((resolve) => {
			...
			LeaderTpuService.load(tpuClient.connection, websocketUrl).then(
				(leaderTpuService) => {
	 ...
	 * @returns
	 */
	async sendAbortableTransaction(
		transaction: Transaction | VersionedTransaction,
		signersOrOptions: Array<Signer> | SendOptions,
		_options?: SendOptions
	): Promise<{
		...
		if (transaction.nonceInfo) {
			...
			const rawTransaction = transaction.serialize();
			const { signature, abortControllers } =
				await this.sendAbortableRawTransaction(rawTransaction);
	 ...
	 * @returns
	 */
	async sendSignedRawTransactionToQuicAddress(
		tpu_address: string,
		logger: Logger,
		webcrypto: peculiarWebcrypto.Crypto,
		...
		try {
			...
			if (!abortController.signal.aborted) {
				...
				if (retryCount < retryMaxCount) {
					return await this.sendSignedRawTransactionToQuicAddress(
						tpu_address,
						logger,
						webcrypto,
						rawTransaction,
	 ...
	 * @returns
	 */
	async sendAbortableRawTransaction(
		rawTransaction: Buffer | number[] | Uint8Array
	): Promise<{
		signature: TransactionSignature;
		abortControllers: AbortController[];
	}> {
		// Extract signature based on whether it's a legacy or versioned transaction
		let signature: string;
		try {
			// Try parsing as a legacy transaction first
			const message = Transaction.from(rawTransaction);
			signature = base58.encode(Uint8Array.from(message.signature));
		} catch (error) {
			// If that fails, try parsing as a versioned transaction
			const versionedTransaction = VersionedTransaction.deserialize(
				Buffer.from(rawTransaction)
			);
			signature = base58.encode(
				Uint8Array.from(versionedTransaction.signatures[0])
			);
		}

		const tpu_addresses = await this.leaderTpuService.leaderTpuSockets(
			this.fanoutSlots
		);
		...
		const abortControllers = tpu_addresses.map((tpu_address) => {
			...
			this.sendSignedRawTransactionToQuicAddress(
				tpu_address,
				logger,
				webcrypto,
				rawTransaction,
		...
		return new Promise((resolve) => {
			...
			leaderTpuService.connection.getSlot('processed').then((start_slot) => {
				...
				LeaderTpuCache.load(connection, start_slot).then((leaderTpuCache) => {
					...
					if (websocket_url !== '') {
						leaderTpuService.subscription = connection.onSlotUpdate(
							(slotUpdate) => {
...
export class TpuConnection extends Connection {
	...
	sendTransaction(
		transaction: Transaction | VersionedTransaction,
		signers: Array<Signer> | SendOptions,
	...
	sendAbortableTransaction(
		...
		return this.tpuClient.sendAbortableTransaction(
			transaction,
			signers,
			sendOptions
		);
	 ...
	 * @returns
	 */
	async sendAndConfirmAbortableTransaction(
		transaction: Transaction | VersionedTransaction,
		signers: Array<Signer> | SendOptions,
		sendOptions?: SendOptions
	): Promise<TransactionSignature> {
		const { signature, abortControllers, blockhash } =
			await this.tpuClient.sendAbortableTransaction(
				transaction,
				signers,
				sendOptions
			);
				...
				if (blockhash) {
					try {
						...
						if (error instanceof TransactionExpiredBlockheightExceededError) {
							return await this.sendAndConfirmAbortableTransaction(
								transaction,
								signers,
								sendOptions
							);
					...
					status = (await this.confirmTransaction(signature, 'processed'))
						.value;
				}
				if (status.err === null) {
					console.log(
						`Transaction Processed https://solscan.io/tx/${signature}`
					);
	 ...
	 * @returns
	 */
	async sendAndConfirmAbortableRawTransaction(
		rawTransaction: Buffer | Array<number> | Uint8Array,
		blockhash?: { blockhash: string; lastValidBlockHeight: number }
	): Promise<TransactionSignature> {
		const { signature, abortControllers } =
			await this.tpuClient.sendAbortableRawTransaction(rawTransaction);
		let status: SignatureResult;
		if (blockhash) {
			status = (
				await this.confirmTransaction({ signature, ...blockhash }, 'processed')
			).value;
		} else {
